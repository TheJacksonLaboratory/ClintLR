...
def extract_correct_diagnosis_from_phenopacket(phenopacket_file):
    ...
    with open(phenopacket_file) as f:
        ...
        for interpretation in phenopacket.interpretations:
            ...
            # TODO -- get the diagnosis and
            fake="MONOD:123"
            if diagnosis_curie is None:
                diagnosis_curie = fake
            elif fake != diagnosis_curie:
                raise ValueError(f"Error: phenopackewt has two different diagnoses!")
    if diagnosis_curie is None:
...
def run_l4ci_and_extract_rank(input_phenopacket, correct_diagnosis):
    """
    TODO -- create minimal arguments list from argparse.
    TODO -- if we open the output file in this function, we need to append, otherwise the previous lines are destroyed
    -- alternatively, return the rank and write the results in the calling code
    """
    # todo - make tempfile
    ...
    arg_list = ["java", "-jar", l4ci_jar, "genes", "-d", data_dir, "-O", outdir, "-M", mondo_path, "-p", phenop]
    command = " ".join(arg_list)
    print(command)
